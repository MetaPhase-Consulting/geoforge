...
import type { AnalysisConfig, AnalysisResult } from '../shared/types.js';
...
export class WebsiteAnalyzer {
  ...
  private async fetchPage(url: string): Promise<string> {
    ...
    for (const proxy of PROXIES) {
      try {
        const response = await this.fetchWithTimeout(`${proxy.url}${encodeURIComponent(url)}`, FETCH_TIMEOUT_MS, '*/*');
        ...
        this.results.technical.contentLength = content.length;
        this.results.errors.push('Fetched through proxy fallback because direct request failed (possible anti-bot restriction).');
        return content;
      } catch {
        // continue
      }
    ...
    throw new Error(
      'Unable to fetch website content. Direct request failed and safe proxy fallback did not return readable HTML.'
    );
  }
  ...
  private isValidFileContent(content: string, expectedType: 'robots' | 'sitemap'): boolean {
    ...
    if (expectedType === 'robots') return /(User-agent:|Disallow:|Allow:|Sitemap:)/i.test(text);
  ...
  private analyzePerformance(): void {
    this.results.performance = {
      loadTime: this.results.technical.responseTime || null,
      domContentLoaded: null,
      firstContentfulPaint: null,
      largestContentfulPaint: null,
      cumulativeLayoutShift: null,
